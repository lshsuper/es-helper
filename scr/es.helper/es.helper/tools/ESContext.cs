...
namespace es.helper.tools
{
    ...
    public class ESContext<T> : ESContextBase<T> where T : class, new()
    {
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public  bool Update(Dictionary<string, object> model, string indexName, string typeName, string id)
        {
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public  bool Update(T model, string indexName, string typeName, string id)
        {
        ...
        /// <returns></returns>
        public  List<ESModel<T>> FindMany(string indexName, string typeName, List<string> ids)
        {

            var result = new List<ESModel<T>>();
            var response = _es.GetMany<T>(ids, indexName, typeName);
            if (response != null)
            {
        ...
        /// <param name="sql"></param>
        /// <returns></returns>
        public  List<ESModel<T>> QueryBySql(string sql)
        {
            using (var client = new HttpClient())
            {
                client.BaseAddress = new Uri("");
               
                HttpContent content = new StringContent(JsonConvert.SerializeObject(new { sql }), Encoding.UTF8, "application/json");
                HttpResponseMessage msg = client.PostAsync("_sql", content).Result;
                string result = msg.Content.ReadAsStringAsync().Result;
                if (msg.StatusCode != HttpStatusCode.OK)
                {
        ...
        /// <returns></returns>
        public  Highlight BuildHightLight(List<string> fields, ESHighLightConfig config)
        {
        ...
        /// <returns></returns>
        public  string BuildHightLightString(string parentStr, string childStr, string preTag, string postTag)
        {
            int equalsCount = 0;
        ...
        /// <returns></returns>
        private  string BuildGroupKey(HashSet<string> groupBys, string tag)
        {
